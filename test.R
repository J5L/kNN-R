...
colnames(wine) <- c("classifier", "alcohol", "malic_acid", "ash", 
                    "alcalinity_of_ash", "magnesium", "total_phenols", 
                    "flavanoids", "nonflavanoid_phenols", "proanthocyanins", 
                    "color_intensity", "hue", "OD280/OD315_of_diluted_wines",
                    "proline")
wine$classifier <- as.factor(wine$classifier)

sample <- sample.int(n = nrow(wine), size = floor(0.75*nrow(wine)), replace = F)
winetrain <- wine[sample, ]
winetest  <- wine[-sample, ]

################################################################################
#algorithm with n-dimensional data
...
#algorithm with 2-dimnesional data
wineKNN2<-myKNN(Xtrain=winetrain, Xtest=winetest, label = "classifier", 
                k=5, var1 = "alcohol", var2 = "malic_acid")
